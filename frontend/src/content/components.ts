...
function createFloatCard(start, end, reason, videoId, videoTitle) {
  ...
  const FLOATCARD_TIME = `From ${secondToStamp(start)} to ${secondToStamp(
    end
  )}:`
  ...
  Object.assign(img, {
    src: chrome.runtime.getURL('float.png'),
    alt: 'tip',
    onclick: handleClick,
  ...
  function handleClick(event) {
    const zoom = videoContainer.querySelector<HTMLElement>(
    ...
    if (chevron) {
      chevron.click()
      waitForPromise('#secondary #placeholder-area', document.body).then(
        (holder) => holder.click()
      )
      waitForPromise('#secondary #contenteditable-root', document.body).then(
        (edit) => {
      ...
      feedBack(
        ...
        videoTitle,
        'iconStamps',
        secondToStamp(video.currentTime)
      )
      setTimeout(() => {
        window.scroll(0, 0)
      }, 1000)
    } else {
      waitForPromise('#primary #placeholder-area', document.body).then(
        (holder) => holder.click()
      )
      waitForPromise('#primary #contenteditable-root', document.body).then(
        (edit) => {
          edit.append(' ' + secondToStamp(video.currentTime))
          edit.focus()
        }
...
function readComments(videoId, commentsTimed, {start, end}) {
  ...
  commentsTimed.sort((a, b) => b.score - a.score)
...
function createAddTimeCard(parent: HTMLElement) {
  const TIMECARD =
    'Consider adding a timestamp to make your comment more accessible!'
...
function createClarifyCard(parent: HTMLElement) {
  const CLARIFYCARD =
    'Try to avoid using ambiguous pronouns, use the full names or concepts instead.'
...
function editStartCard(captions, comments, keywords, parent: HTMLElement) {
  ...
  comments.sort(
    (a, b) => stampToSecond(a.timestamps[0]) - stampToSecond(b.timestamps[0])
  )
    ...
    comments.forEach((comment) => {
      ...
      comment.timestamps.forEach((timestamp) => {
        ...
        stampElement.onclick = () => {
          ...
          video.play()
          waitForPromise(
            '#secondary #contenteditable-root',
            document.body
          ).then((edit) => {
            edit.append(' ' + timestamp)
            edit.focus()
...
function createRangeBar(blackRanges, thresh) {
  ...
  blackRanges.forEach((blackRange, i) => {
    ...
    color =
      score < thresh1 ? palette[0] : score < thresh2 ? palette[1] : palette[2]
...
function createAccordion(commentsTimed, parent) {
  ...
  allTimestamps.forEach((timestamp) => {
    ...
    const comments = commentsTimed.filter(
      (comment) => comment.timestamps[0] == timestamp
    )
    comments.forEach((comment) => {
